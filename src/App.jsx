...
function decodeInt32LEtoGrams(dataView) {
  const mg = dataView.getInt32(0, true) // little-endian
  return mg / 1000
}
...
export default function App() {
  ...
  const [baselineAbs, setBaselineAbs] = useState(null) // tare software
  ...
  async function connect() {
    try {
      ...
      const device = await navigator.bluetooth.requestDevice({
        filters: [{ namePrefix: 'MOTIF' }, { namePrefix: 'Mentor' }],
  ...
  async function start() {
    ...
    const onNotify = (event) => {
      ...
      let base = baselineAbs
      if (base === null) {
        base = absoluteG
        setBaselineAbs(base)
      }
      const g = absoluteG - base
      ...
      lastSampleRef.current = { t, g }
      setCurrentG(g)
      setFlowGps(flow)
      setSamples(prev => [...prev, { t, g }])
    }
    ...
    abortRef.current = () => {
      ...
      charRef.current?.removeEventListener('characteristicvaluechanged', onNotify)
    }
  }

  function tareNow() {
    // re-baseline usando la lectura absoluta más reciente
    setBaselineAbs(prev => (prev === null ? 0 : prev + currentG))
  }
  ...
  const chartOptions = useMemo(() => ({
    ...
    scales: {
      x: { title: { display: true, text: 'Tiempo (s)' }, ticks: { maxTicksLimit: 8 } },
      y: { title: { display: true, text: 'g' } }
    }
  }), [])

  function downloadCSV() {
    const header = 't_s,peso_g\n'
    const rows = samples.map(s => `${s.t.toFixed(3)},${s.g.toFixed(3)}`).join('\n')
    const blob = new Blob([header + rows], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'espresso_weight_timeseries.csv'; a.click()
    ...
    <div className="container">
      <div className="card">
        <div className="row" style={{justifyContent:'space-between'}}>
          <h2 style={{margin:0}}>Mentor Coffee Scale • Web Bluetooth</h2>
        ...
        <div className="row">
          ...
          <button className="primary" disabled={connecting||connected} onClick={connect}>
        ...
        <div className="grid">
          <div className="card" style={{padding:'16px'}}>
            <div className="sub">Peso actual</div>
            <div className="metric">{currentG.toFixed(2)} <span className="sub">g</span></div>
            <div className="sub">Flow (dW/dt): {flowGps.toFixed(2)} g/s</div>
          </div>
          <div className="card" style={{padding:'16px'}}>
            <div className="sub">Dispositivo</div>
            <div style={{fontWeight:700}}>{deviceName || '—'}</div>
            <div className="sub">Service: {serviceUUID}</div>
            <div className="sub">Char: {charUUID}</div>
          </div>
        </div>
        ...
        <div className="footer">
          Consejo: Conecta y elige “MOTIF SCALE”. La primera muestra se usa como TARE (baseline software). Requiere Chrome/Edge con HTTPS.
        </div>
      </div>
    </div>
  )
